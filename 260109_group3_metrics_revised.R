...
# 0. Basic settings ----
if (dir.exists("/home/zygao02/wuhua_proj/260109/") == F) dir.create("/home/zygao02/wuhua_proj/260109/")
setwd("/home/zygao02/wuhua_proj/260109/")
...
group3 <- readRDS("/home/lushi02/project/wuhua/group3_res0.8.rds")

ciToti8 <- subset(group3, orig.ident %in% c("ciToti8"))
D_E75 <- subset(group3, orig.ident %in% c("D_E75"))
EPSC_S7 <- subset(group3, orig.ident %in% c("EPSC_S7"))
ETiX6 <- subset(group3, orig.ident %in% c("ETiX6"))
iEFCEM_day6 <- subset(group3, orig.ident %in% c("iEFCEM_day6"))
TFSEM_day8 <- subset(group3, orig.ident %in% c("TFSEM_day8"))
...
group3_merged <- merge(x = ciToti8, 
                       y = c(D_E75, EPSC_S7, ETiX6, iEFCEM_day6, TFSEM_day8),
                       add.cell.ids = c("ciToti8", "D_E75", "EPSC_S7", "ETiX6", "iEFCEM_day6", "TFSEM_day8"))

group3_merged_qc <- qc_pipeline(group3_merged, "group3_raw_merge")
group3_merged_qc[["percent.mt"]] <- PercentageFeatureSet(group3_merged_qc, pattern = "^mt-")

group3_merged_qc <- NormalizeData(group3_merged_qc) |> 
...
group3_harmony <- harmony_pipe(group3_merged_qc, reso = 1.5)

umap_pipeline(seurat_obj = group3_harmony,
              group_label = "group3_harmony", 
              plot_title_group = "3",
              plot_width = 15, plot_height = 8)



p <- DimPlot(group3_harmony, reduction = "umap", split.by = "orig.ident",
             group.by = "cell_type",
             pt.size = 0.5, 
             alpha = 0.6,
             label = TRUE,
...
combined_plot_split <- (p) + 
  plot_annotation(
    title = paste0("Split UMAP for Group 3"),
    theme = theme(
      plot.title = element_text(size = 24, face = "bold", hjust = 0.5)
    ) )

ggsave(paste0("1.UMAP_group3_harmony_split.pdf"), plot = combined_plot_split, width = 38, height = 7)
...
# output
outdir <- "group3_metrics_3metrics"
