...
keep_ct <- sort(unique(as.character(group2$cell_type)))
keep_ct <- keep_ct[!is.na(keep_ct)]

group2_ref_filt <- subset(group2_ref, subset = !is.na(cell_type) & cell_type %in% keep_ct)
group2_ref_filt$cell_type <- droplevels(factor(group2_ref_filt$cell_type))

train <- group2_ref_filt
...
## ---------------------------
if (dir.exists("/home/zygao02/wuhua_proj/260106/") == F) dir.create("/home/zygao02/wuhua_proj/260106/")
...
if (!is.null(best_obj)) {
  ...
  pdf(file.path(outdir, "BEST_heatmap.pdf"),
  ...
  write.csv(best_mat, file.path(outdir, "BEST_heatmap.csv"),
            row.names = TRUE, quote = FALSE)
  
  save(
    list = c("group2_ref_filt", "train", "test",
...
outdir <- "/home/zygao02/wuhua_proj/260106"
mat_plot <- as.matrix(read.csv(file.path(outdir, "BEST_heatmap.csv"),
...
outdir <- "glm_topN_around6000_group2"
