...
# exlude cluster 6 & re-clustering
regress_harmony_sub <- subset(regress_harmony, seurat_clusters != 6)

regress_harmony_sub <- FindClusters(regress_harmony_sub, resolution = 0.06)
table(regress_harmony_sub$seurat_clusters)

umap_pipeline(seurat_obj = regress_harmony_sub,
              group_label = "regress_harmony_noC6", 
              plot_title_group = "7",
              plot_width = 12, plot_height = 6)

saveRDS(regress_harmony_sub, "regress_harmony_sub.rds")
...
library(Hmisc)
group1_ref <- readRDS("/home/lushi02/project/wuhua/ref/group1ref_E3.5_E4.5.rds")
sub_regress_harmony <- subset(regress_harmony_sub, orig.ident %nin% c("ciToti1", "ciToti2", "ciToti3"))
...
pdf("glm_predict_similarity_heatmap_noC6_scale.pdf",
    width = pdf_w_in, height = pdf_h_in, useDingbats = FALSE)
...
saveRDS(regress_harmony_sub, "regress_harmony_noC6.rds")


regress_harmony_sub_markers <- FindAllMarkers(regress_harmony_sub, assay = "RNA",
                                          layer = "data", group.by = "seurat_clusters")

write.csv(regress_harmony_markers, "regress_harmony_noC6_markers.csv", quote = F, row.names = T)
