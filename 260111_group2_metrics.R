...
# 0. Basic settings ----
if (dir.exists("/home/zygao02/wuhua_proj/260111/") == F) dir.create("/home/zygao02/wuhua_proj/260111/")
setwd("/home/zygao02/wuhua_proj/260111/")
...
group2 <- readRDS("/home/lushi02/project/wuhua/group2_final.rds")

ciToti7 <- subset(group2, orig.ident %in% c("ciToti7"))
ETiX5 <- subset(group2, orig.ident %in% c("ETiX5"))
D_E65 <- subset(group2, orig.ident %in% c("D_E65"))
...
group2_merged <- merge(x = ciToti7, 
                       y = c(ETiX5, D_E65),
                       add.cell.ids = c("ciToti7", "ETiX5", "D_E65"))

group2_merged_qc <- qc_pipeline(group2_merged, "group2_raw_merge")
group2_merged_qc[["percent.mt"]] <- PercentageFeatureSet(group2_merged_qc, pattern = "^mt-")

group2_merged_qc <- CellCycleScoring(group2_merged_qc, 
                                     s.features = str_to_title(cc.genes$s.genes), 
                                     g2m.features = str_to_title(cc.genes$g2m.genes), 
                                     set.ident = TRUE)

group2_merged_qc <- NormalizeData(group2_merged_qc) |> 
  FindVariableFeatures(nfeatures = 2000) |> 
  ScaleData(vars.to.regress = c("percent.mt", "S.Score", "G2M.Score")) |> 
  RunPCA(npcs = 50)

group2 <- RunHarmony(group2_merged_qc, 
                     group.by.vars = "orig.ident", 
                     reduction = "pca", 
                     dims.use = 1:20, 
                     reduction.save = "harmony")

group2 <- RunUMAP(group2, reduction = "harmony", dims = 1:20) |> 
  FindNeighbors(reduction = "harmony", dims = 1:20) |> 
  FindClusters(resolution = 0.5)

umap_pipeline(seurat_obj = group2,
              group_label = "group2", 
              plot_title_group = "2",
...
combined_plot_split <- (p4) + 
  plot_annotation(
    title = paste0("Split UMAP for Group 2"),
    theme = theme(
      plot.title = element_text(size = 24, face = "bold", hjust = 0.5)
    ) )

ggsave(paste0("1.UMAP_group2_split.pdf"), plot = combined_plot_split, width = 15, height = 4)
...
# output
outdir <- "group2_metrics_3metrics"
...
run_metric_pair_global_sym_boot <- function(
  ...
  if (!is.finite(n_each) || n_each <= 1) return(NULL)
...
add_global_sym_and_replot <- function(
  ...
  cap_term <- if (is.finite(cap_each_sym_kbet_ilisi)) cap_each_sym_kbet_ilisi else Inf
  
  common_each <- min(
    as.numeric(counts[[ref_use]]),
    as.numeric(counts[[baseline]]),
    min(as.numeric(counts[models_all]), na.rm = TRUE),
    if (metric_use == "asw") floor(cap_total_global_asw/2) else cap_term,
    na.rm = TRUE
  )
  
  n_each_fixed <- as.integer(common_each)
  
  # 防御：避免 NA/太小
  if (!is.finite(n_each_fixed) || n_each_fixed <= 1) n_each_fixed <- NULL
